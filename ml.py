...
#                     recommendations = get_recommendations(peliculas_5, similarity_matrix, dfre) 
            ...
            def calcular_puntuacion(row):
                if row["type"] == "MOVIE":
                        ...
                        return (
                            0.77 * (row['Puntuacionge'] - dfre['Puntuacionge'].min()) / (dfre['Puntuacionge'].max() - dfre['Puntuacionge'].min()) +
                            0.10 * row["Puntuacionac"] / dfre["Puntuacionac"].max()+
                            0.13 *(row['imdb_score'] - dfre['imdb_score'].min()) / (dfre['imdb_score'].max() - dfre['imdb_score'].min())
            )
            ...
            st.dataframe(recomendaciones1[['title', 'platform']].head(10))


            st.write("Aquí tienes algunas recomendaciones de series para ti:")
            st.dataframe(recomendaciones2[['title', 'platform']].head(10))
